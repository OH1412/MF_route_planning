    ...
    if path_states:
        ...
        for idx_k1, k1_node in enumerate(kfs1_list):
            ...
            if comp_t and all(not (act[0][0]=="pick_kfs1_ext" and act[0][1]==k1_node) for act in actions):
                ...
                order += 1
                ext_path_segments.append((order, k1_node, ext_start_idx, initial_ext_pos, arrival_time, completion_time))
...
def draw_grid_with_pink_ring(ax, heights: Dict[int, int], path: Optional[List[int]],
    ...
    for r in range(OUT_ROWS):
        for c in range(OUT_COLS):
            ...
            inner = outer_to_inner_node(r, c)
            if inner is not None:
                ax.text(c + 0.12, OUT_ROWS - 1 - r + 0.75, f'{inner}', ha='left', va='center', fontsize=9, fontweight='bold', color='white')

    # 绘 KFS1 / KFS2 / KFS_locked
    for node in kfs1_set:
        r_out, c_out = inner_to_outer_coord(node)
        row_disp = OUT_ROWS - 1 - r_out
        ax.add_patch(plt.Rectangle((c_out+0.2, row_disp+0.2), 0.6, 0.6, facecolor='#ffbb44', edgecolor='black', linewidth=1.2))
    for i, node in enumerate(kfs2_positions):
        r_out, c_out = inner_to_outer_coord(node)
        row_disp = OUT_ROWS - 1 - r_out
        bit = 1 << i
        if (picked_mask_kfs2 & bit):
            ax.add_patch(plt.Rectangle((c_out+0.28, row_disp+0.28), 0.44, 0.44, facecolor='#f0e6ff', edgecolor='purple', linewidth=1.0))
        elif (cleaned_mask_kfs2 & bit):
            ax.add_patch(plt.Rectangle((c_out+0.26, row_disp+0.26), 0.48, 0.48, facecolor='#e6f2ff', edgecolor='blue', linewidth=1.0))
        else:
            ax.add_patch(plt.Rectangle((c_out+0.2, row_disp+0.2), 0.6, 0.6, facecolor='#aa66cc', edgecolor='black', linewidth=1.2))

    if kfs_locked is not None:
        rr_out, cc_out = inner_to_outer_coord(kfs_locked)
        row_disp = OUT_ROWS - 1 - rr_out
        ax.add_patch(plt.Rectangle((cc_out+0.2, row_disp+0.2), 0.6, 0.6, facecolor='#444444', edgecolor='black', linewidth=1.2))
    ...
    for (order, k1_node, from_idx, to_idx, arrival_time, completion_time) in ext_path_segments:
        ...
        for i in range(len(ring_indices)-1):
            a_idx = ring_indices[i]; b_idx = ring_indices[i+1]
            a_pos = PINK_RING[a_idx]; b_pos = PINK_RING[b_idx]
            ax_x1 = a_pos[1] + 0.5; ax_y1 = OUT_ROWS - 1 - a_pos[0] + 0.5
            ax_x2 = b_pos[1] + 0.5; ax_y2 = OUT_ROWS - 1 - b_pos[0] + 0.5
        ...
        tx = to_pos[1] + 0.5; ty = OUT_ROWS - 1 - to_pos[0] + 0.2
    ...
    for (order, node, dur, comp_time) in kfs1_pick_events:
        r_out, c_out = inner_to_outer_coord(node)
        row_disp = OUT_ROWS - 1 - r_out
        ax.text(c_out + 0.8, row_disp + 0.15, f'#{order}\n{dur:.2f}s\nc={comp_time:.2f}s', ha='center', va='center', fontsize=7,
                bbox=dict(facecolor='white', alpha=0.85, edgecolor='none', pad=1))

    ax.set_xlim(0, OUT_COLS); ax.set_ylim(0, OUT_ROWS); ax.set_aspect('equal'); ax.axis('off')
...
if __name__ == "__main__":
    ...
    for idx, kfs_locked in enumerate(kfs_locked_candidates):
        ...
        time_pick_kfs1_map = {node: time_pick_kfs1_map_global.get(node, times["time_pick_kfs1_default"]) for node in kfs1_set}
        ...
        ax.text(-0.53, 0.02, f'locked={kfs_locked}\nKFS1={sorted(kfs1_set)}\nKFS2={sorted(kfs2_positions)}',
                transform=ax.transAxes, fontsize=7, ha='left', va='bottom', bbox=dict(facecolor='white', alpha=0.6, edgecolor='none'))

        if node_path is None:
            ax.text(2.5, 3.0, '不可达', ha='center', va='center', fontsize=12, color='red', fontweight='bold', zorder=10)
        else:
            ax.text(7.0, 5.6, f't={display_time:.2f}s\npicked={picked_cnt}', ha='right', va='top', fontsize=9, bbox=dict(facecolor='white', edgecolor='none', pad=1), zorder=10)
    ...
    legend_elements = [
        ...
        Line2D([0], [0], color='w', marker='s', markerfacecolor='#f0e6ff', markeredgecolor='purple', markersize=6, label='KFS2（已拾取）'),
        Line2D([0], [0], color='w', marker='s', markerfacecolor='#e6f2ff', markeredgecolor='blue', markersize=6, label='KFS2（已清除但未拾取）'),
        Line2D([0], [0], color='w', marker='s', markerfacecolor='#ffe6ef', markersize=7, label='粉色跑道（外部机器人活动区域）'),
